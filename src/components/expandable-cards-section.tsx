...
export default function ExpandableCardsSection() {
  const [activeCard, setActiveCard] = useState<number | null>(3) // Default to the 4th card (index 3)
  ...
  useEffect(() => {
    ...
    contentRefs.current.forEach((content, index) => {
      ...
      if (activeCard === index) {
        gsap.fromTo(content, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, delay: 0.2, ease: "power2.out" })
      } else {
        ...
        {cardData.map((card, index) => (
          <div
            ...
            className={`relative rounded-2xl p-6 md:p-8 cursor-pointer transition-all
                       ...
                       ${activeCard === index ? "bg-black text-white" : "bg-[#f7f7f7] text-black"}
                       overflow-hidden`}
            style={{
              flex: isMobile ? "1 1 auto" : activeCard === index ? (isTablet ? 2 : 3) : 1,
            }}
          >
            <span className="text-6xl md:text-8xl font-bold opacity-80">{card.number}</span>
            ...
            <div
              ...
              className={`mt-4 text-base md:text-lg
                         ${activeCard === index ? "block" : "hidden md:block md:opacity-0"}`}
            >
